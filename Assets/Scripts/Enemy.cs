...
public class Enemy : LivingEntity
{
   ...
   [Range(0.01f, 2f)] public float trunSmoothTime = 0.1f;
   ...
   private List<LivingEntity> lastAttackedTarets = new List<LivingEntity>();
   ...
   private bool hastarget => targetEntity != null && !targetEntity.dead;
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        {
            var leftRayRotation = Quaternion.AngleAxis(-fieldOfVeiw * 0.5f, Vector3.up);
            var leftRayDirection = leftRayRotation * transform.forward;
            Handles.color = new Color(1f, 1f, 1f, 0.2f);
            Handles.DrawSolidArc(eyeTransform.position, Vector3.up, leftRayDirection, fieldOfVeiw, viewDistance);
        }
    }
#endif
    ...
    private void FixedUpdate()
    {
        ...
        if(state == State.AttackBegin || state == State.Attacking)
        {
            ...
            targetAngleY = Mathf.SmoothDampAngle(transform.eulerAngles.y, targetAngleY, ref turnSmoothVelocity, trunSmoothTime);
        ...
        if(state == State.Attacking)
        {
            ...
            for(var i = 0; i < size; i++)
            {
                ...
                if(attackTargetEntity != null && !lastAttackedTarets.Contains(attackTargetEntity))
                {
                    ...
                    lastAttackedTarets.Add(attackTargetEntity);
                    break;
                }
            }
        }
    ...
    private bool IsTargetOnSight(Transform target)
    {
        ...
        if(Vector3.Angle(direction, eyeTransform.forward) > fieldOfVeiw * 0.5f)
        {
            return false;
        }
